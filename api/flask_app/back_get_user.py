...
from models import products, purchases, users
...
def get_user(index):
    ...
    if count < index:
        ...
        try:
            sub_query = db.session.query(User, func.row_number()).label("row_number")
            sub_query = sub_query.subquery()
            user_array = db.session.query(sub_query).filter(sub_query.c.row_number >= index & sub_query.c.row_number < index + 10)
